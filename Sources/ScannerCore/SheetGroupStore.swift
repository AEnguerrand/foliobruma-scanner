...
public struct SheetGroupStore {
  public var groups: [SheetGroup]
  public var snapshot: Data?

  public init(groups: [SheetGroup], snapshot: Data?) {
    self.groups = groups
  ...
  public mutating func save(_ group: SheetGroup, in root: URL, locking: any LibraryLocking) throws {
    ...
    let next = try locking.withLock(at: root.appendingPathComponent("sheet-groups.lock")) {
      ...
      return (groups: next, snapshot: data)
    }
    groups = next.groups
...
public struct SheetRecord: Identifiable {
